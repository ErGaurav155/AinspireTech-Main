...
export const Checkout = ({
  ...
  const renderWebsiteModal = () => (
    <AlertDialog open={showModal} onOpenChange={setShowModal}>
      <AlertDialogContent
        className={`${isDark ? "bg-[#1A1A1E] border border-white/[0.08]" : "bg-white border border-gray-100"} rounded-2xl max-w-md p-0 overflow-hidden shadow-xl`}
      >
        <motion.div
        ...
        >
          ...
          {currentStep === "weblink" ? (
            ...
            >
              <div className="text-center">
                <h3
                  className={`text-lg font-bold ${styles.text.primary} text-center`}
                >
                  {getStepTitle()}
                </h3>
                <p
                  className={`text-sm ${styles.text.secondary} text-center mt-1`}
                >
                  {getStepDescription()}
                </p>
              </div>
              ...
              <Button
                type="submit"
                disabled={isSubmitting || processing}
                className={`w-full py-3 bg-gradient-to-r from-pink-500 to-rose-500 hover:from-pink-600 hover:to-rose-600 text-white rounded-xl font-medium ${isSubmitting || processing ? "opacity-50 cursor-not-allowed" : ""}`}
              >
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="space-y-4">
                <div
                  className={`${isDark ? "bg-white/[0.03]" : "bg-gray-50"} rounded-xl p-4`}
                >
                  <div className="flex items-center space-x-3">
                    <div className="flex-shrink-0">
                      {scrapingComplete && chatbotCreationComplete ? (
                        ...
                        <Loader2
                          className={`w-6 h-6 ${isDark ? "text-pink-400" : "text-pink-500"} animate-spin`}
                        />
                      )}
                    </div>
                ...
                {scrapingComplete && chatbotCreationComplete && (
                  <div
                    className={`${isDark ? "bg-green-500/10 border border-green-500/20" : "bg-green-50 border border-green-200"} rounded-xl p-4`}
                  >
                    <p
                      className={`${isDark ? "text-green-400" : "text-green-600"} text-sm flex items-center`}
                    >
            ...
            <div className="p-12 text-center">
              <div className="flex flex-col items-center justify-center space-y-4">
                <Loader2
                  className={`w-8 h-8 ${isDark ? "text-pink-400" : "text-pink-500"} animate-spin`}
                />
          ...
          <div className={`p-4 text-center border-t ${styles.divider}`}>
            <AlertDialogDescription className={`text-xs ${styles.text.muted}`}>
              {getFooterText()}
            </AlertDialogDescription>
          </div>
        </motion.div>
