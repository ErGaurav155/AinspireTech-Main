...
export default function CallMarketingPage() {
  ...
  return (
    <main className={`min-h-screen max-w-7xl mx-auto px-4 py-8 ${themeStyles.page}`}>
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center py-10 md:py-16">
        <motion.div
          variants={stagger}
          initial="hidden"
          whileInView="visible"
        ...
        >
          ...
          >
            ...
            <br />
            <span className={gradientText}>Turn calls into qualified leads</span>
          </motion.h1>
          ...
          <motion.div variants={fadeUp} className="space-y-2 font-montserrat">
            {[
              { icon: <PhoneCall className="h-5 w-5" />, text: "24/7 inbound answering" },
              { icon: <Users className="h-5 w-5" />, text: "Caller details and lead capture" },
              { icon: <Bell className="h-5 w-5" />, text: "WhatsApp, SMS, and email alerts" },
              { icon: <ShieldCheck className="h-5 w-5" />, text: "Call logs, summaries, and transcripts" },
            ].map((feature, index) => (
              <motion.div
                key={feature.text}
                whileHover={{ x: 5 }}
          ...
          <motion.div variants={fadeUp} className="flex flex-col sm:flex-row gap-4">
            <motion.div whileHover={{ scale: 1.05, y: -2 }} whileTap={{ scale: 0.95 }}>
              <Button
                asChild
                className="bg-gradient-to-r from-[#00F0FF] to-[#FF2E9F] text-black font-bold py-6 px-6 rounded-2xl hover:shadow-2xl transition-all duration-300"
              >
                <Link href={`${dashboardUrl}/call`}>
            ...
            <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
              <Button
                asChild
                variant="outline"
                className={`border-2 font-semibold py-6 px-6 rounded-2xl transition-all duration-300 ${themeStyles.outline}`}
              >
                <Link href="/call/pricing">
        ...
        >
          ...
          >
            {[
              { label: "Lead saved", icon: CheckCircle2, value: "Amit Sharma" },
              { label: "Duration", icon: Clock3, value: "3m 04s" },
              { label: "Language", icon: Languages, value: "Hindi + English" },
      ...
      <section className="py-12">
        ...
        >
          ...
          <p className={`mt-3 text-base md:text-lg font-montserrat ${themeStyles.muted}`}>
            The public page sells the promise, the dashboard handles the work:
            calls, leads, numbers, flows, billing, notifications, team, and integrations.
          </p>
        </motion.div>
        ...
        <div className="mt-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {features.map((feature) => (
            ...
            >
              ...
              <p className={`mt-2 text-sm leading-relaxed font-montserrat ${themeStyles.muted}`}>
                {feature.description}
              </p>
            </motion.div>
          ))}
        ...
        >
          ...
          <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-3">
            {[
              ...
              <div key={item.title} className={`rounded-2xl border p-4 ${themeStyles.inner}`}>
                <item.icon className="h-5 w-5 text-[#00F0FF]" />
      ...
      <section className="py-12">
        <div className="text-center mb-8">
          <h2 className="text-3xl md:text-4xl font-bold">Pricing comparison</h2>
          <p className={`mt-3 font-montserrat ${themeStyles.muted}`}>
            Three clear plans for call volume, team size, and receptionist coverage.
          </p>
        </div>
        <div className={`overflow-x-auto rounded-2xl border ${themeStyles.card}`}>
          <table className="w-full min-w-[720px] border-collapse">
            <thead className={currentTheme === "dark" ? "bg-white/[0.05]" : "bg-gray-50"}>
              <tr>
                {["Feature", "Starter", "Growth", "Enterprise"].map((heading) => (
                  <th key={heading} className="border border-white/10 px-5 py-4 text-left">
                    {heading}
                  </th>
                ))}
              </tr>
      ...
      <section className="py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {reviews.map((review) => (
            ...
            >
              <p className={`font-montserrat text-sm leading-relaxed ${themeStyles.muted}`}>
                "{review.quote}"
              </p>
              <div className="mt-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {faqs.map((faq) => (
            ...
            >
              ...
              <p className={`mt-2 font-montserrat text-sm leading-relaxed ${themeStyles.muted}`}>
                {faq.answer}
              </p>
            </motion.div>
          ))}
      ...
      >
        ...
        <p className={`mt-3 max-w-2xl mx-auto font-montserrat ${themeStyles.muted}`}>
          Start with a receptionist flow, connect your number, and capture leads
          from calls your team used to miss.
        </p>
